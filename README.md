...
graph LR
    ...
    %% We use a darker theme background to match GitHub Dark Mode
    %% "minlen" forces longer arrows to make the diagram wider/bigger
    
    classDef default fill:#0d1117,stroke:#c9d1d9,stroke-width:2px,color:#c9d1d9,rx:5,ry:5;
    classDef security fill:#2e001f,stroke:#ff00bf,stroke-width:2px,color:#fff,rx:5,ry:5;
    classDef ai fill:#001a2e,stroke:#00bfff,stroke-width:2px,color:#fff,rx:5,ry:5;
    classDef db fill:#2e2800,stroke:#ffd700,stroke-width:3px,color:#fff,rx:10,ry:10;
    classDef user fill:#1f6feb,stroke:#fff,stroke-width:3px,color:#fff,rx:10,ry:10;

    %% --- 1. THE INGESTION ZONE (LEFT) ---
    subgraph Ingestion [" 📂  SECURE INGESTION PIPELINE "]
        direction TB
        Docs["📄 Raw Documents"]:::default -->|1. Extract| PII_Scan["👁️ PII Detector"]:::security
        PII_Scan -->|2. Redact| PII_Mask["🛡️ Anonymizer"]:::security
        PII_Mask -->|3. Clean| Chunker["✂️ Semantic Chunker"]:::default
        Chunker -->|4. Vectorize| Embed["🧠 Private Embeddings"]:::ai
    end

    %% --- THE BRIDGE (CENTER DATABASE) ---
    %% We place this outside the subgraphs so it acts as the bridge
    Embed == "💾 Store Safe Vectors" ==> VectorDB[("🗄️ Vector DB (RBAC)")]:::db

    %% --- 2. THE RETRIEVAL ZONE (RIGHT) ---
    subgraph Retrieval [" 🤖  RAG INFERENCE PIPELINE "]
        direction TB
        User["👤 Authorized User"]:::user -->|Query| API["⚡ API Gateway"]:::default
        API -->|1. Check| Guard["🛑 Input Guardrails"]:::security
        
        Guard -- "2. Search (with Role)" --> VectorDB
        VectorDB -- "3. Retrieve Chunks" --> ReRanker["⚖️ Cross-Encoder"]:::ai
        
        ReRanker -->|4. Filter| Context["📝 Context Window"]:::default
        Context -->|5. Prompt| LLM_Gate["BRIDGE (LiteLLM)"]:::default
        LLM_Gate -->|6. Generate| LLM["🧠 Enterprise LLM"]:::ai
        
        LLM -->|7. Reply| Deanonymize["🔓 De-Anonymizer"]:::security
        Deanonymize -->|8. Audit| Audit["📜 Audit Log"]:::security
    end

    %% Final Return Path
    Audit -.->|Response| API

    %% --- FORCE STYLING FOR ZONES ---
    %% This makes the boxes huge, dark, and clear
    style Ingestion fill:#161b22,stroke:#30363d,stroke-width:4px,stroke-dasharray: 5 5,color:#fff
    style Retrieval fill:#161b22,stroke:#30363d,stroke-width:4px,stroke-dasharray: 5 5,color:#fff
    
    %% --- LINK STYLING ---
    %% Make arrows thicker and clearer
    linkStyle default stroke:#777,stroke-width:3px;
  ```
